...
namespace WebSocket.Portable.Compression
{
    ...
    internal class HuffmanTree
    {
        ...
        private static readonly HuffmanTree _staticLiteralLengthTree;
        private static readonly HuffmanTree _staticDistanceTree;

        static HuffmanTree()
        {
            // construct the static literal tree and distance tree
            _staticLiteralLengthTree = new HuffmanTree(GetStaticLiteralTreeLength());
            _staticDistanceTree = new HuffmanTree(GetStaticDistanceTreeLength());
        }
        ...
        public HuffmanTree(byte[] codeLengths)
        {
            ...
            _tableBits = _codeLengthArray.Length == MaxLiteralTreeElements ? 9 : 7;
        ...
        static byte[] GetStaticDistanceTreeLength()
        {
            ...
            for (var i = 0; i < MaxDistTreeElements; i++)
            {
        ...
        private void CreateTable()
        {
            ...
            uint[] codeArray = this.CalculateHuffmanCode();
            ...
            for (var ch = 0; ch < _codeLengthArray.Length; ch++)
            {
                // length of this code
                ...
                if (len <= _tableBits)
                {
                    ...
                    var increment = 1 << len;
                    if (start >= increment)
                        throw new InvalidDataContractException("Huffman data is invalid.");

                    // Note the bits in the table are reverted.
                    var locs = 1 << (_tableBits - len);
                ...
                else
                {
                    ...
                    var overflowBits = len - _tableBits;    // the nodes we need to respent the data.
                    var codeBitMask = 1 << _tableBits;    // mask to get current bit (the bits can't fit in the table)  
                    ...
                    var index = start & ((1 << _tableBits) - 1);
                    var array = _table;
                    ...
                    do
                    {
                        ...
                        array = (start & codeBitMask) == 0 ? _left : _right;
        ...
        public int GetNextSymbol(InputBuffer input)
        {
            ...
            {       //  this will be the start of the binary tree
                // navigate the tree
                var mask = (uint)1 << _tableBits;
                do
                {
                    symbol = -symbol;
                    symbol = (bitBuffer & mask) == 0 ? _left[symbol] : _right[symbol];
                    mask <<= 1;
                } while (symbol < 0);
            }
