...
namespace WebSocket.Portable.Compression
{
    internal class FastEncoderWindow
    {
        ...
        public int FreeWindowSpace
        {
            ...
            get { return 2 * FastEncoderWindowSize - _bufEnd; }
        }
        ...
        public void MoveWindows()
        {
            ...
            {
                var val = _lookup[i] - FastEncoderWindowSize;
            ...
            {
                var val = ((long)_prev[i]) - FastEncoderWindowSize;
        ...
        private static uint HashValue(uint hash, byte b)
        {
        ...
        //
        private int FindMatch(int search, out int matchPos, int searchDepth)
        {
            ...
            var bestMatch = 0; // best match length found so far
            var bestMatchPos = 0; // absolute match position of best match found

            // the earliest we can look
            var earliest = _bufPos - FastEncoderWindowSize;
            ...
            Debug.Assert(bestMatch < MinMatch || matchPos < FastEncoderWindowSize,
        ...
        {
            for (var i = 0; i < FastEncoderHashtableSize; i++)
            {
                var where = _lookup[i];
                ...
                while (where != 0 && _bufPos - where < FastEncoderWindowSize)
                {
                    ...
                    var nextWhere = _prev[@where & FastEncoderWindowMask];
